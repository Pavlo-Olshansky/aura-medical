...
import { getAqiColor, formatWeatherDateTime, createWeatherChartOptions } from '@/utils/weatherUtils'

const props = defineProps<{
  data: AirQualityData
}>()

const aqiColor = computed(() => getAqiColor(props.data.aqi))
...
const chartData = computed(() => {
  ...
  return {
    labels: forecast.map(p => formatWeatherDateTime(p.measured_at)),
    datasets: [{
      label: 'AQI',
...
const chartOptions = createWeatherChartOptions({ yMin: 1, yMax: 5, yStepSize: 1, maxXTicks: 8 })
</script>
